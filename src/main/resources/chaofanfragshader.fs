...
vec3 mapLightDiffuse(vec3 input0)
{
    /*vec3 input = input0 + 0.1;
    float steps = 3.0;
    float interval = 1.0 / steps;
    float rampSmooth = 0.05;
    vec3 level = round(input * steps) / steps;
    vec3 ramp = interval * smoothstep(level - rampSmooth * interval * 0.5, level + rampSmooth * interval * 0.5, input) + level - interval;
    return ramp;*/
    return 0.3 + smoothstep(0.48, 0.52, input0) * 0.3 + smoothstep(0.65, 0.67, input0) * 0.4;
}
...
void main() {
	    ...
	    vec3 lightDiffuse = mapLightDiffuse(v_lightDiffuse);
		#if defined(ambientFlag) && defined(separateAmbientFlag)
			#ifdef shadowMapFlag
